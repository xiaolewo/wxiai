...
def test_veo_user_config():
    ...
    try:
        ...
        print(
            f"📊 当前配置: enabled={config.enabled}, default_model={config.default_model}"
        )
        ...
        print("\n💰 测试模型积分配置...")
        model_credits = (
            config.model_credits_config or config._get_default_model_credits()
        )
        ...
        critical_checks = [
            ...
            (
                "default_model",
                user_config["default_model"] is not None
                and user_config["default_model"] != "",
            ),
            ("supported_models", len(user_config["supported_models"]) > 0),
            ("model_credits_config", len(user_config["model_credits_config"]) > 0),
            (
                "default_model_in_supported",
                user_config["default_model"] in user_config["supported_models"],
            ),
            (
                "default_model_has_credits",
                user_config["default_model"] in user_config["model_credits_config"],
            ),
        ]
        ...
        if user_config["default_model"] not in user_config["supported_models"]:
            print(
                f"❌ 默认模型 '{user_config['default_model']}' 不在支持的模型列表中！"
            )
