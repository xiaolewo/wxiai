...
async def test_http_action_request():
    ...
    try:
        ...
        with get_db() as db:
            ...
            task = db.query(VeoTask).filter(
                VeoTask.status.in_(["processing", "submitted", "failed", "completed"])
            ).first()
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                try:
                    ...
                    print(f"🌐 【HTTP测试】响应数据: {json.dumps(response_data, indent=2, ensure_ascii=False)}")
...
async def test_direct_endpoint():
    ...
    try:
        ...
        with get_db() as db:
            ...
            task = db.query(VeoTask).filter(
                VeoTask.status.in_(["processing", "submitted"])
            ).first()
