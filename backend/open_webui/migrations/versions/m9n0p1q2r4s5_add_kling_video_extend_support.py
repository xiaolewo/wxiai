...
def upgrade() -> None:
    ...
    op.add_column(
        "kling_tasks", sa.Column("parent_task_id", sa.String(length=50), nullable=True)
    )
    ...
    # 4. 添加索引提升查询性能
    op.create_index(
        op.f("ix_kling_tasks_parent_task_id"), "kling_tasks", ["parent_task_id"]
    )

    op.create_index(op.f("ix_kling_tasks_is_extended"), "kling_tasks", ["is_extended"])
