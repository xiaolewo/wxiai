...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # 默认参数配置
        sa.Column(
            "default_model", sa.String(length=50), nullable=False, default="veo3"
        ),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_veo_tasks_id"), table_name="veo_tasks")
    op.drop_table("veo_tasks")

    # 删除Veo配置表
    op.drop_index(op.f("ix_veo_config_id"), table_name="veo_config")
