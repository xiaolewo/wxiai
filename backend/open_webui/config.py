...
def _ensure_jimeng_fields():
    ...
    try:
        ...
        with get_db() as db:
            ...
            create_all_tables()
            # 添加字段修复
            if inspector.has_table("mj_tasks"):
                columns = inspector.get_columns("mj_tasks")
                column_names = [col["name"] for col in columns]

                # 添加 cloud_image_url 字段
                if "cloud_image_url" not in column_names:
                    log.info("➕ 添加 cloud_image_url 字段到 mj_tasks...")
                    db.execute(
                        text("ALTER TABLE mj_tasks ADD COLUMN cloud_image_url VARCHAR")
                    )
                    db.commit()
                    log.info("✅ cloud_image_url 字段添加成功")

            # 检查 dreamwork_tasks 表
            if inspector.has_table("dreamwork_tasks"):
                columns = inspector.get_columns("dreamwork_tasks")
                column_names = [col["name"] for col in columns]

                # 添加 cloud_image_url 字段
                if "cloud_image_url" not in column_names:
                    log.info("➕ 添加 cloud_image_url 字段到 dreamwork_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE dreamwork_tasks ADD COLUMN cloud_image_url VARCHAR"
                        )
                    )
                    db.commit()
                    log.info("✅ cloud_image_url 字段添加成功")

            # 检查 google_images_credits 表
            if inspector.has_table("google_images_credits"):
                columns = inspector.get_columns("google_images_credits")
                column_names = [col["name"] for col in columns]

                # 添加 credits_before 字段
                if "credits_before" not in column_names:
                    log.info("➕ 添加 credits_before 字段到 google_images_credits...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_credits ADD COLUMN credits_before INTEGER"
                        )
                    )
                    db.commit()
                    log.info("✅ credits_before 字段添加成功")

                # 添加 credits_after 字段
                if "credits_after" not in column_names:
                    log.info("➕ 添加 credits_after 字段到 google_images_credits...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_credits ADD COLUMN credits_after INTEGER"
                        )
                    )
                    db.commit()
                    log.info("✅ credits_after 字段添加成功")

                # 添加 model_name 字段
                if "model_name" not in column_names:
                    log.info("➕ 添加 model_name 字段到 google_images_credits...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_credits ADD COLUMN model_name VARCHAR"
                        )
                    )
                    db.commit()
                    log.info("✅ model_name 字段添加成功")

                # 添加 description 字段
                if "description" not in column_names:
                    log.info("➕ 添加 description 字段到 google_images_credits...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_credits ADD COLUMN description VARCHAR"
                        )
                    )
                    db.commit()
                    log.info("✅ description 字段添加成功")

            # 检查 veo_config 表
            if inspector.has_table("veo_config"):
                columns = inspector.get_columns("veo_config")
                column_names = [col["name"] for col in columns]

                # 添加 model_credits_config 字段
                if "model_credits_config" not in column_names:
                    log.info("➕ 添加 model_credits_config 字段到 veo_config...")
                    db.execute(
                        text(
                            "ALTER TABLE veo_config ADD COLUMN model_credits_config TEXT"
                        )
                    )
                    db.commit()
                    log.info("✅ model_credits_config 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 query_interval 字段
                if "query_interval" not in column_names:
                    log.info("➕ 添加 query_interval 字段到 veo_config...")
                    db.execute(
                        text("ALTER TABLE veo_config ADD COLUMN query_interval INTEGER")
                    )
                    db.commit()
                    log.info("✅ query_interval 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 default_enhance_prompt 字段
                if "default_enhance_prompt" not in column_names:
                    log.info("➕ 添加 default_enhance_prompt 字段到 veo_config...")
                    db.execute(
                        text(
                            "ALTER TABLE veo_config ADD COLUMN default_enhance_prompt INTEGER DEFAULT 1 NOT NULL"
                        )
                    )
                    db.commit()
                    log.info("✅ default_enhance_prompt 字段添加成功")

                # 添加 default_model 字段
                if "default_model" not in column_names:
                    log.info("➕ 添加 default_model 字段到 veo_config...")
                    db.execute(
                        text(
                            "ALTER TABLE veo_config ADD COLUMN default_model INTEGER DEFAULT 1 NOT NULL"
                        )
                    )
                    db.commit()
                    log.info("✅ default_model 字段添加成功")

                # 添加 max_concurrent_tasks 字段
                if "max_concurrent_tasks" not in column_names:
                    log.info("➕ 添加 max_concurrent_tasks 字段到 veo_config...")
                    db.execute(
                        text(
                            "ALTER TABLE veo_config ADD COLUMN max_concurrent_tasks INTEGER DEFAULT 3 NOT NULL"
                        )
                    )
                    db.commit()
                    log.info("✅ max_concurrent_tasks 字段添加成功")

            # 检查 google_images_tasks 表
            if inspector.has_table("google_images_tasks"):
                columns = inspector.get_columns("google_images_tasks")
                column_names = [col["name"] for col in columns]

                # 添加 input_images 字段
                if "input_images" not in column_names:
                    log.info("➕ 添加 input_images 字段到 google_images_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN input_images JSON"
                        )
                    )
                    db.commit()
                    log.info("✅ input_images 字段添加成功")

                    log.info("🎬 即梦字段检查完成")
                # 添加 cloud_input_images 字段
                if "cloud_input_images" not in column_names:
                    log.info("➕ 添加 cloud_input_images 字段到 google_images_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN cloud_input_images JSON"
                        )
                    )
                    db.commit()
                    log.info("✅ cloud_input_images 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 cloud_result_images 字段
                if "cloud_result_images" not in column_names:
                    log.info(
                        "➕ 添加 cloud_result_images 字段到 google_images_tasks..."
                    )
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN cloud_result_images TEXT"
                        )
                    )
                    db.commit()
                    log.info("✅ cloud_result_images 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 images 字段
                if "images" not in column_names:
                    log.info("➕ 添加 images 字段到 google_images_tasks...")
                    db.execute(
                        text("ALTER TABLE google_images_tasks ADD COLUMN images JSON")
                    )
                    db.commit()
                    log.info("✅ images 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 n 字段
                if "n" not in column_names:
                    log.info("➕ 添加 n 字段到 google_images_tasks...")
                    db.execute(
                        text("ALTER TABLE google_images_tasks ADD COLUMN n INTEGER")
                    )
                    db.commit()
                    log.info("✅ n 字段添加成功")
                ...
                if "submit_time" not in column_names:
                    log.info("➕ 添加 submit_time 字段到 google_images_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN submit_time DATETIME"
                        )
                    )
                    db.commit()
                    log.info("✅ submit_time 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 start_time 字段
                if "start_time" not in column_names:
                    log.info("➕ 添加 start_time 字段到 google_images_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN start_time DATETIME"
                        )
                    )
                    db.commit()
                    log.info("✅ start_time 字段添加成功")

                    log.info("🎬 即梦字段检查完成")

                # 添加 cloud_image_urls 字段
                if "cloud_image_urls" not in column_names:
                    log.info("➕ 添加 cloud_image_urls 字段到 google_images_tasks...")
                    db.execute(
                        text(
                            "ALTER TABLE google_images_tasks ADD COLUMN cloud_image_urls DATETIME"
                        )
                    )
                    db.commit()
                    log.info("✅ cloud_image_urls 字段添加成功")
