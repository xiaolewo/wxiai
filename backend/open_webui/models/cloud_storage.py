...
def _ensure_cloud_storage_tables():
    """确保云存储相关表存在"""
    ...
    try:
        ...
        if missing_tables:
            print(f"🔧 检测到缺失的云存储表: {missing_tables}")

            # 创建表
            Base.metadata.create_all(
                db.bind, tables=[CloudStorageConfig.__table__, GeneratedFile.__table__]
            )
            print("🎨 云存储表已创建")

            # 检查是否需要创建默认配置
            config_count = db.execute(
                text("SELECT COUNT(*) FROM cloud_storage_config")
            ).scalar()
            if config_count == 0:
                # 插入默认配置
                db.execute(
                    ...
                    ('default', 'tencent-cos', 0, '', '', 'ap-beijing', '', '', 
                     1, :allowed_types, 104857600, 'generated/', 'images/', 'videos/')
                """
                    ),
                    {"allowed_types": json.dumps(["image/*", "video/*"])},
                )
                db.commit()
                print("🎨 默认云存储配置已创建")

    except Exception as e:
        print(f"🔧 云存储表检查过程中出错: {e}")
    finally:
        db.close()
...
# 应用启动时自动检查表
try:
    _ensure_cloud_storage_tables()
except Exception as e:
    print(f"🔧 云存储表自动检查失败: {e}")
