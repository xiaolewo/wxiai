...
class VeoConfig(Base):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "model_credits_config": self.model_credits_config or self._get_default_model_credits(),
    ...
    def get_model_image_limits(self, model_name: str) -> dict:
        ...
        limits = {
            ...
            "veo2-fast-components": {"max": 3, "description": "最多支持3张图(视频元素)"},
    ...
    def _ensure_tables_exist(cls):
        ...
        with get_db() as db:
            ...
            # 创建索引（如果不存在）
            try:
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_config_id ON veo_config (id)"))
            except:
                pass
            ...
            # 创建veo_tasks表索引
            try:
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_tasks_id ON veo_tasks (id)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_tasks_user_id ON veo_tasks (user_id)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_tasks_status ON veo_tasks (status)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_tasks_created_at ON veo_tasks (created_at)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_tasks_external_task_id ON veo_tasks (external_task_id)"))
            except:
                pass
            ...
            # 创建veo_credits表索引
            try:
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_credits_id ON veo_credits (id)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_credits_user_id ON veo_credits (user_id)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_credits_task_id ON veo_credits (task_id)"))
                db.execute(text("CREATE INDEX IF NOT EXISTS ix_veo_credits_created_at ON veo_credits (created_at)"))
            except:
                pass
