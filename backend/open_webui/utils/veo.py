...
class VeoApiClient:
    ...
    async def generate_video(self, request: VeoGenerateRequest) -> dict:
        ...
        logger.debug(
            f"🎬 【Veo客户端】请求payload: {json.dumps(payload, ensure_ascii=False, indent=2)}"
        )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(
                    f"🎬 【Veo客户端】收到响应: status_code={response.status_code}"
                )
                ...
                if not response.is_success:
                    ...
                    logger.error(
                        f"🎬 【Veo客户端】HTTP错误 {response.status_code}: {error_text}"
                    )
                    return {
                        "success": False,
                ...
                logger.info(
                    f"🎬 【Veo客户端】API响应解析成功: code={result.get('code')}, message={result.get('message', '')}"
                )

                if result.get("code") == "success":
                    logger.info(
                        f"🎬 【Veo客户端】API调用成功: task_id={result.get('data')}"
                    )
                    return {
                        "success": True,
                    ...
                    logger.error(
                        f"🎬 【Veo客户端】API返回失败: {result.get('message', '生成失败')}"
                    )
                    return {
                        "success": False,
            ...
            error_msg = (
                f"无法连接到Veo API服务器 ({self.base_url})，请检查网络连接或服务器状态"
            )
...
def validate_user_credits(user_id: str, required_credits: int) -> bool:
    ...
    try:
        ...
        logger.info(
            f"💰 【Veo积分】用户 {user_id} 当前积分: {current_balance}, 需要积分: {required_credits}"
        )
...
def deduct_user_credits(
    user_id: str, credits: int, task_id: str, model_name: str = None
) -> bool:
    """扣除用户积分"""
    try:
        user_credits = Credits.get_user_credits(user_id)
        ...
        if success:
            ...
            logger.info(
                f"💰 【Veo积分】用户 {user_id} 扣除积分 {credits}，余额: {credits_before} -> {credits_after}"
            )
            return True
        else:
...
def add_user_credits(
    user_id: str, credits: int, task_id: str, reason: str = "退费"
) -> bool:
    """返还用户积分"""
    try:
        user_credits = Credits.get_user_credits(user_id)
        ...
        if success:
            ...
            logger.info(
                f"💰 【Veo积分】用户 {user_id} 返还积分 {credits}，余额: {credits_before} -> {credits_after}"
            )
            return True
        else:
...
async def process_veo_generation(request: VeoGenerateRequest, user_id: str) -> dict:
    ...
    try:
        logger.info(
            f"🎬 【Veo处理】开始处理用户 {user_id} 的生成请求，task_id: {task_id}"
        )
        logger.info(
            f"🎬 【Veo处理】请求参数: model={request.model}, prompt='{request.prompt[:100]}...', enhance_prompt={request.enhance_prompt}"
        )
        ...
        logger.info(
            f"🎬 【Veo处理】配置获取成功: enabled={config.enabled}, base_url={config.base_url}"
        )
        ...
        if request.images:
            ...
            for i, image in enumerate(request.images):
                ...
                cloud_url = await upload_image_to_cloud(
                    image, user_id, f"{task_id}_{i}"
                )
                if cloud_url:
                    logger.info(
                        f"🎬 【Veo处理】图片 {i+1} 上传成功: {cloud_url[:50]}..."
                    )
        ...
        logger.info(
            f"🎬 【Veo处理】API请求准备完成: images_count={len(cloud_input_images) if cloud_input_images else 0}"
        )
        ...
        logger.info(
            f"🎬 【Veo处理】Veo API返回结果: success={api_result.get('success')}, message='{api_result.get('message', '')}'"
        )
        ...
        if api_result["success"]:
            logger.info(
                f"🎬 【Veo处理】API调用成功，external_task_id: {api_result['task_id']}"
            )
            # 更新任务状态
            ...
            VeoTask.update_task_status(
                task_id, {"status": "failed", "fail_reason": api_result["error"]}
            )
    ...
    while attempt < max_attempts:
        try:
            ...
            if not result["success"]:
                logger.error(
                    f"🔍 【Veo监控】查询任务 {task_id} 失败: {result['error']}"
                )
                VeoTask.update_task_status(
                    task_id, {"status": "failed", "fail_reason": result["error"]}
                )
                break
            ...
            logger.debug(
                f"🔍 【Veo监控】任务 {task_id} 状态: {status}, 进度: {progress}"
            )
            ...
            if status == "SUCCESS":
                ...
                if video_url:
                    ...
                    update_data.update(
                        {"status": "failed", "fail_reason": "未获取到视频结果"}
                    )
                    VeoTask.update_task_status(task_id, update_data)
                    break
                ...
                if task:
                    add_user_credits(
                        task.user_id, task.credits_cost, task_id, "生成失败"
                    )
