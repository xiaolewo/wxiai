    ...
    async def generate_image(self, request: GoogleImagesGenerateRequest) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(
                    f"🎨 【谷歌生图】准备发送请求，数据字段: {list(data.keys())}, 文件数量: {len(files)}"
                )
                ...
                logger.info(
                    f"🎨 【谷歌生图】收到响应: status_code={response.status_code}"
                )
                ...
                if not response.is_success:
                    ...
                    logger.error(
                        f"🎨 【谷歌生图】HTTP错误 {response.status_code}: {error_text}"
                    )
                    return {
                        "success": False,
                ...
                try:
                    ...
                    return {"success": False, "error": "响应格式错误，无法解析JSON数据"}
...
def validate_user_credits(user_id: str, required_credits: int) -> bool:
    ...
    try:
        ...
        logger.info(
            f"💰 【谷歌生图积分】用户 {user_id} 当前积分: {current_balance}, 需要积分: {required_credits}"
        )
...
def deduct_user_credits(
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"💰 【谷歌生图积分】用户 {user_id} 扣除积分 {credits}，余额: {credits_before} -> {credits_after}"
            )
            return True
        else:
...
def add_user_credits(
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"💰 【谷歌生图积分】用户 {user_id} 返还积分 {credits}，余额: {credits_before} -> {credits_after}"
            )
            return True
        else:
...
async def upload_image_to_cloud(
    image_data: str, user_id: str, task_id: str, index: int = 0
) -> Optional[str]:
    """上传图片到云存储"""
    try:
        ...
        if image_data.startswith("http"):
            ...
            if success and file_record and file_record.cloud_url:
                logger.info(
                    f"📁 【谷歌生图云存储】图片上传成功: {file_record.cloud_url}"
                )
...
async def process_google_images_generation(
    request: GoogleImagesGenerateRequest, user_id: str
) -> dict:
    """处理谷歌生图生成请求"""
    ...
    try:
        logger.info(
            f"🎨 【谷歌生图处理】开始处理用户 {user_id} 的生成请求，task_id: {task_id}"
        )
        ...
        if request.images:
            logger.info(
                f"🎨 【谷歌生图处理】开始处理 {len(request.images)} 张输入图片..."
            )
