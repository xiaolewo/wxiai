...
@router.post("/config")
async def save_google_images_config(config_data: GoogleImagesConfigForm, user=Depends(get_admin_user)) -> dict:
    """保存谷歌生图配置 - 管理员专用"""
    try:
        # 验证必需字段
        if config_data.enabled and (not config_data.base_url or not config_data.api_key):
            raise HTTPException(
                status_code=400,
                detail="启用服务时需要提供基础URL和API密钥"
            )
...
async def get_google_images_user_config(user=Depends(get_verified_user)) -> GoogleImagesUserConfig:
...
async def create_image_edit(request: GoogleImagesGenerateRequest, user=Depends(get_verified_user)) -> dict:
    ...
    try:
        ...
        if result["success"]:
            ...
            return {
                ...
                "data": [
                    {"url": url} if not url.startswith("data:image/") else {"b64_json": url.split(",")[1]}
...
async def generate_google_images(request: GoogleImagesGenerateRequest, user=Depends(get_verified_user)) -> GoogleImagesGenerateResponse:
    """谷歌生图生成接口 - 自定义格式"""
    try:
        # 检查服务是否启用
        config = GoogleImagesConfig.get_config()
...
async def get_google_images_task(task_id: str, user=Depends(get_verified_user)) -> GoogleImagesTaskResponse:
    """获取任务详情"""
    try:
        task = GoogleImagesTask.get_task_by_id(task_id)
        if not task:
            return GoogleImagesTaskResponse(
                success=False,
                error="任务不存在"
            )
...
async def delete_google_images_task(task_id: str, user=Depends(get_verified_user)) -> dict:
    """删除任务"""
    try:
        task = GoogleImagesTask.get_task_by_id(task_id)
        if not task:
    ...
    try:
        ...
        for credit in credits:
            credit_list.append({
                ...
                "created_at": credit.created_at.isoformat() if credit.created_at else ""
            })
        
        return {
            "success": True,
            "credits": credit_list,
            "total": len(credit_list)
        }
    
    except Exception as e:
        return {
            "success": False,
            "error": f"获取积分历史失败: {str(e)}"
        }
...
async def get_google_images_admin_stats(user=Depends(get_admin_user)) -> dict:
    ...
    try:
        ...
        with get_db() as db:
            ...
            # 成功任务数
            completed_tasks = db.query(GoogleImagesTask).filter(
                GoogleImagesTask.status == "completed"
            ).count()
            
            # 失败任务数
            failed_tasks = db.query(GoogleImagesTask).filter(
                GoogleImagesTask.status == "failed"
            ).count()
            
            # 今日任务数
            from datetime import date
            today_tasks = db.query(GoogleImagesTask).filter(
                func.date(GoogleImagesTask.created_at) == date.today()
            ).count()
            
            # 总积分消耗
            total_credits = db.query(
                func.sum(GoogleImagesCredit.credit_amount)
            ).filter(
                GoogleImagesCredit.operation_type == "deduct"
            ).scalar() or 0
