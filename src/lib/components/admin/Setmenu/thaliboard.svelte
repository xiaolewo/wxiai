<script>
	...
	import { updateUserRole, getUsers, deleteUserById } from '$lib/apis/setmenu';

	import Pagination from '$lib/components/common/Pagination.svelte';
	import ChatBubbles from '$lib/components/icons/ChatBubbles.svelte';
	import Tooltip from '$lib/components/common/Tooltip.svelte';

	import EditUserModal from '$lib/components/admin/Setmenu/UserList/EditUserModal.svelte';
	import UserChatsModal from '$lib/components/admin/Setmenu/UserList/UserChatsModal.svelte';
	import AddUserModal from '$lib/components/admin/Setmenu/UserList/AddUserModal.svelte';
	...
	const getUserList = async () => {
		try {
			...
			if (res) {
				...
				//       features: [],
				//       credits: 500,
				//       created_at: 1747985046,
				//       updated_at: 1747985046
				//     }
				//   ]
				// }
				users = res.plans ?? [];
	...
	<div class="mt-0.5 mb-2 gap-1 flex flex-col md:flex-row justify-between">
		...
		<div class="flex gap-1">
			<div class=" flex w-full space-x-2">
				...
				<div>
					<Tooltip content={$i18n.t('addmenu')}>
						<button
			...
			<tbody class="">
				{#each users as user, userIdx}
					<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
						...
						<td class=" px-3 py-1"> {user.description} </td>
						...
						<td class=" px-3 py-1"> {user.is_active ? '启用' : '禁用'} </td>
						...
						<td class="px-3 py-1 text-right">
							<div class="flex justify-end w-full">
								...
								<Tooltip content={$i18n.t('editmenu')}>
									<button
										class="self-center w-fit text-sm px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
										on:click={async () => {
								...
								{#if user.role !== 'admin'}
									<Tooltip content={$i18n.t('deletemenu')}>
										<button
											class="self-center w-fit text-sm px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
											on:click={async () => {
