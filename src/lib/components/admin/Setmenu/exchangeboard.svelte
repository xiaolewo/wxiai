<script>
	...
	import { updateUserRole, getSubCodes, deleteUserById, getUsers } from '$lib/apis/setmenu';

	import Pagination from '$lib/components/common/Pagination.svelte';
	import ChatBubbles from '$lib/components/icons/ChatBubbles.svelte';
	import Tooltip from '$lib/components/common/Tooltip.svelte';

	import EditUserModal from '$lib/components/admin/Setmenu/Board/EditUserModal.svelte';
	import UserChatsModal from '$lib/components/admin/Setmenu/Board/UserChatsModal.svelte';
	import AddUserModal from '$lib/components/admin/Setmenu/Board/AddUserModal.svelte';
	...
	<div class="mt-0.5 mb-2 gap-1 flex flex-col md:flex-row justify-between">
		...
		<div class="flex gap-1">
			<div class=" flex w-full space-x-2">
				...
				<div>
					<Tooltip content={$i18n.t('exchangetext-generate')}>
						<button
		...
		>
			<div class="flex gap-1.5 items-center">
				...
				{/if}
					...
					>
						<div class="flex gap-1.5 items-center">
							{$i18n.t('exchangetext-updated_at')}
			...
			<tbody class="">
				{#each users as user, userIdx}
					<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
						...
						<td class=" px-3 py-1"> {user.used_at} </td>
						<td class=" px-3 py-1">
							{dayjs(user.created_at * 1000).format('LL')}
						</td>

						<td class=" px-3 py-1"> {dayjs(user.expires_at * 1000).format('LL')}</td>
